...
function loadNotes(number) {
	...
	let textFilter = document.getElementById("searchInput").value;
	...
	if (typeof localStorage.allNotes != 'undefined' ) {
		...
		if (textFilter != "") {
			...
			dbNotes.map(content => {
					...
					let note = content.note.toLowerCase();

					if (title.indexOf(textFilter) != -1 | note.indexOf(textFilter) != -1 ) {
						filter.push(dbNotes.indexOf(content));
					}
					console.log(filter)
				})
		...
		console.log(number + " primeiro number")
		...
		console.log(number + " segundo number")


		if (filter.at(-1) >= 10 /*& filter.length >= 10*/) {
			//console.log(filter.at(number))
			...
			if (number <= 10) {
				...
				if (number >= filter.length) {
					...
					number = filter.at(number - 1);

				}
			}
			console.log(number + " number com filter")
		...
		for (let i = 0; i <= number; i++) {
			...
			if (textFilter != "" & filter.indexOf(i) == -1) {
				color += "; display: none"
			}
		...
		+ '<img src="./css/loadMore.svg" style="height: 28px" onclick="loadNotes('+ number +')">'
		+ '</div>'
		+ '</div>';
			...
			if (number >= filter.at(-1) & textFilter != "") {

				console.log(number + " hide")
				document.getElementById("moreNotes").style= "display: none";
				
			}

			if (textFilter != "" & filter.length == "0") {
				document.getElementById("moreNotes").style= "display: none";
